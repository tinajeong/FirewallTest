...
$portArray = $args[1] -split ","
$timeout = $args[2]
$ProgressPreference = 'SilentlyContinue'

Write-Host "=====FIREWALL CHECK START====="
Write-Host "=====Timeout :$timeout ms====="
foreach ($server in $serverArray){
    foreach ($port in $portArray) {
        if(Test-PSOnePort -ComputerName $server -Port $port -Timeout $timeout) {"$server $port is opened"} else {"$server $port is closed"}
    }
}

Write-Host "=====FIREWALL CHECK END====="